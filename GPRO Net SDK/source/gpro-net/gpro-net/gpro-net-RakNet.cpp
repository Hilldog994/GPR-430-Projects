...
namespace gproNet
{
	...
	RakNet::BitStream& cRakNetManager::WritePose(RakNet::BitStream& bitstream, sSpatialPose const pose)
	{
		WriteTimestamp(bitstream);
		bitstream.Write((RakNet::MessageID)ID_GPRO_MESSAGE_SENDPOSEDATA);
		bitstream.Write(pose);
		return bitstream;
	}

	RakNet::BitStream& cRakNetManager::ReadPose(RakNet::BitStream& bitstream)
	{
		sSpatialPose pose;
		bitstream.Read(pose);

		//do correction algorithms here if thats a thing that needs to be done

		poses[pose.clientsID][pose.poseID] = pose;
		return bitstream;
	}
